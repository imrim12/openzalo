...
3.  **Decide Structure**:
    *   **Simple Plan**: Single markdown file in `.claude/plans/`.
    *   **Detailed Plan**: Folder structure in `.claude/plans/` with separate phase files.
...
Format: `.claude/plans/plan-<plan_short_summarization>-<Random ID>.md`

**Detailed Plan (High Impact)**:
Format: `.claude/plans/<plan_short_summarization>-<Random ID>/`
